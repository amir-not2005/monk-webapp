...
const cardFeatureHtml = featuresText.map((featureText) => {
  return (
    <div
      key={featureText.id}
      className="px-6 py-6 md:shadow-2xl md:border-none border-b-4 border-slate-800"
    >
      <h1 className="font-bold text-3xl mb-3">{featureText.title}</h1>
      <p>{featureText.content}</p>
    </div>
  );
});
